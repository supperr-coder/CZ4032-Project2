...
class Apriori():
    ...
    def generate_new_combinations_low_memory(self, old_combinations, X, min_support, is_sparse):
        """
        Generator of all combinations based on the last state of Apriori algorithm
        Parameters
        -----------
        old_combinations: np.array
            All combinations with enough support in the last step
        def  Combinations are represented by a matrix.
            Number of columns is equal to the combination size
            of the previous step.
            Each row represents one combination
            and contains item type ids in the ascending order
    ...
    def apriori(
        self, df, min_support=0.5, use_colnames=False, max_len=None, verbose=0, low_memory=False, count = False
    ):
        ...
        while max_itemset and max_itemset < (max_len or float("inf")):
            ...
            if low_memory:
                combin = self.generate_new_combinations_low_memory(
